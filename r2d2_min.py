...
class Actor:
    ...
    def _rollout(self) -> (list, list):
        ...
        # a0の形状を(batch_size, )に変更します。
        a0 = np.array([seg.a_init for seg in segments])  # (batch_size, )

        # a0を2次元に拡張し、actionsと同じ形状にします。
        a0 = np.expand_dims(a0, axis=0)  # (1, batch_size)

        # 修正後のa0とactionsを結合します。
        prev_actions = np.vstack([a0, actions])[:-1]  # (burnin+unroll_len, batch_size)
